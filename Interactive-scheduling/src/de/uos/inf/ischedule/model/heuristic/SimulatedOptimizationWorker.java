...
/**
 * A worker for simulating an optimization procedure.
 * 
 * @author David Meignan
 */
public class SimulatedOptimizationWorker extends OptimizationWorker {
	 ...
	 * @param finalSolution the solution to return.
	 ...
	 */
	public SimulatedOptimizationWorker(ShiftSchedulingProblem problem,
			Solution finalSolution, IteratedLocalSearchSettings settings) {
		if (problem == null || settings == null || finalSolution == null)
	...
	public synchronized boolean updateBestFound(Solution solution) {
		if (solution == null)
			return false;
		if (bestFoundSolution == result)
			return false; // Do nothing
		if (bestFoundSolution == null ||
				bestFoundSolution.getEvaluation().compareTo(solution.getEvaluation()) > 0) {
			if (solution.getEvaluation().compareTo(result.getEvaluation()) <= 0) {
				bestFoundSolution = result;
			} else {
